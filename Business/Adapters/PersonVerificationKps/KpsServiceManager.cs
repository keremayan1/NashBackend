...
namespace Business.Adapters.PersonVerificationKps
{
    public class KpsServiceManager : IKpsService
    {
        ...
        public async Task<bool> VerifyId(Person person)
        {
            ...
            var kpsVerification = await kps.TCKimlikNoDogrulaAsync(Convert.ToInt64(person.NationalId), person.FirstName.ToUpper(), person.LastName.ToUpper(), person.DateOfBirth.Year);
